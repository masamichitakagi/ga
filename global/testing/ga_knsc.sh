...
omp_num_threads=1
ppn=4
async_progress_pin=8,9,18,19
lpp=1 # logical-per-physical
ncpu_mt=16 # number of CPUs for main-thread

while getopts srga:n:mp:dN:P:M:ho:v:A: OPT
do
        case ${OPT} in
	    s) stop=1
		;;
...
if [ ${go} -eq 1 ]; then
	...
	mpiexec.hydra -l -n $nprocs -ppn $ppn -machinefile $PBS_NODEFILE $ilpopt $MCEXEC $mcexecopt ./$exe 
    elif [ "$mpitype" == "openmpi" ]; then
